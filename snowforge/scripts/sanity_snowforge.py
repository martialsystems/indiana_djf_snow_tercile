...
from snowforge._bootstrap import ensure_paths  # noqa: E402
...
def main() -> int:
    ...
    for law in discover_product_laws(REPO / "snowforge"):
        ...
        require_law(
            ...
            law_id=str(law.get("id") or "snow_law"),
            thread_id=f"sanity_{law.get('id')}",
            raise_error=True,
        )
    print("product_laws: OK")
    tr = subprocess.run(
        [sys.executable, "-m", "pytest", str(REPO / "tests" / "test_snowforge_laws.py"), "-q"],
        cwd=str(REPO),
        check=False,
    )
